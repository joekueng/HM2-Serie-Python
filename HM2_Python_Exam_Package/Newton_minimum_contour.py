...
# Suchbegriffe / parole chiave esame:
# Newton-Verfahren fuer Systeme ohne Daempfung
# Newton-Verfahren für Systeme ohne Dämpfung
# Minimalstelle, Minimum, Gradient, grad g = 0, contour, f1=0, f2=0
...
def plot_zero_contours(x_min=0.0, x_max=1.0, y_min=0.0, y_max=1.0):
    ...
    plt.figure()
    contour_1 = plt.contour(x_grid, y_grid, f1_values, levels=[0], colors="tab:red")
    contour_2 = plt.contour(x_grid, y_grid, f2_values, levels=[0], colors="tab:blue")
    handles_1, _ = contour_1.legend_elements()
    handles_2, _ = contour_2.legend_elements()
    plt.legend([handles_1[0], handles_2[0]], ["f1 = dg/dx = 0", "f2 = dg/dy = 0"])
    plt.xlabel("x")
    plt.ylabel("y")
    plt.grid(True)
    plt.title("Graphische Loesung: f1 = 0 und f2 = 0")


if __name__ == "__main__":
    # Adattare da plot o testo esame.
    start_vector = np.array([0.2, 0.8], dtype=float)
    tolerance = 1e-8

    plot_zero_contours()
    solution, iterations = newton_system(f, jacobian, start_vector, tolerance)

    print("\nRESULT")
    print("solution =", solution)
    print("iterations =", iterations)
    print("f(solution) =", f(solution))
    print("||f(solution)||_inf =", np.linalg.norm(f(solution), ord=np.inf))
